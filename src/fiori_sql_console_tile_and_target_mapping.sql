...
FROM AGR_BUFFI
WHERE MANDT = session_context('CLIENT') AND SUBSTR_REGEXPR('(.+)\:(.+)' IN URL GROUP 1 ) IN ('sap-ui2-group','X-SAP-UI2-CATALOGPAGE')
AND AGR_NAME LIKE 'TS%' 
)
,
...
WHERE "ORIG" . "MANDT" = session_context('CLIENT') 
...
ON J.PARENTID = PT.ID AND PT.MANDT = session_context('CLIENT') AND PT.LANGU = 'E'
